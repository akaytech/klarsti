...
> = (set, get) => ({
  ...
  updateDecisionProjectName: (id, name) => islem(() => {
    const newDecision = (get().decision || []).map(d => d.id === id ? { ...d, name } : d);
    set({ decision: newDecision });
  }),
  deleteDecisionProject: (id) => islem(() => {
    const newDecision = (get().decision || []).filter(d => d.id !== id);
    set({ decision: newDecision });
  }),
  addDecisionCriteria: (projectId, name, weight) => islem(() => {
  ...
  updateDecisionCriteria: (projectId, criteriaId, name, weight) => islem(() => {
    const newDecision = (get().decision || []).map(d => 
      d.id === projectId 
  ...
  deleteDecisionCriteria: (projectId, criteriaId) => islem(() => {
    const newDecision = (get().decision || []).map(d => 
      d.id === projectId 
        ? { 
            ...d, 
  ...
  addDecisionOption: (projectId, name) => islem(() => {
    const newDecision = (get().decision || []).map(d => 
      d.id === projectId 
  ...
  updateDecisionOptionName: (projectId, optionId, name) => islem(() => {
    const newDecision = (get().decision || []).map(d => 
      d.id === projectId 
  ...
  deleteDecisionOption: (projectId, optionId) => islem(() => {
    const newDecision = (get().decision || []).map(d => 
      d.id === projectId 
  ...
  updateDecisionScore: (projectId, optionId, criteriaId, score) => islem(() => {
    const newDecision = (get().decision || []).map(d => 
      d.id === projectId 
        ? { 
            ...d, 
