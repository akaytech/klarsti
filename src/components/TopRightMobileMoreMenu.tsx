...
export default function TopRightMobileMoreMenu() {
  ...
  const { activeTopMenu, setActiveTopMenu, triggerShare, triggerExport, setGuideOpen } = useUIStore(useShallow((state) => ({
    ...
    setGuideOpen: state.setGuideOpen
  })));
  
  const activeTool = useRoadmapStore(state => state.activeTool);
  const setActiveTool = useRoadmapStore(state => state.setActiveTool);
    ...
    <div ref={menuRef} className="absolute top-4 end-4 z-[100] sm:hidden flex flex-col items-end">
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          {/* Geniş ekranda ayrı bir düğme var; burası onun dar ekran karşılığı,
              aynı kuralla görünüyor. */}
          ...
          {/* Geniş ekrandaki kılavuz düğmesinin dar ekran karşılığı. */}
          ...
          {/* Ajanda kişiseldir; ne paylaşılır ne de görsel olarak dışa aktarılır. */}
          {activeTool && activeTool !== 'notepad' && (
            <>
