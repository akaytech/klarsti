...
import { FtaProbabilityContext } from '../utils/ftaProbability';

export default memo(function FtaNode({ data, id }: { data: FtaNodeData; id: string }) {
  const { t } = useTranslation();
  const isRoot = id === 'root';
  const probabilityMap = useContext(FtaProbabilityContext);
  const probability = probabilityMap.get(id);
    ...
    switch (data.type) {
      ...
      case 'andGate':
        return {
          ...
          icon: <span className="font-black text-indigo-500 text-lg me-1">&amp;</span>,
      ...
      case 'priorityAndGate':
        return {
          ...
          icon: <ListOrdered size={16} className="text-violet-500 me-1" />,
          shape: 'rounded-b-[2rem] rounded-t-sm px-4'
        };
      case 'orGate':
        return {
          ...
          icon: <span className="font-black text-teal-500 text-sm me-1">≥1</span>,
      ...
      case 'exclusiveOrGate':
        return {
          ...
          icon: <GitBranch size={16} className="text-cyan-500 me-1" />,
    ...
    >
      ...
      {probability !== undefined && (
        <div className={clsx("mt-2 px-1.5 py-0.5 rounded text-[10px] font-bold inline-block border", 
          ...
          {t('fta_probability_label')} {(probability * 100).toFixed(1)}%
        </div>
      )}

      {data.type !== 'basicEvent' && data.type !== 'undevelopedEvent' && data.type !== 'conditioningEvent' && (
        <Handle 
          type="source" 
