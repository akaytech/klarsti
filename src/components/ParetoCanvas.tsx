...
import { Plus, Trash2, BarChart2, Edit2 } from 'lucide-react';
...
export default function ParetoCanvas() {
  ...
  const {  currentProjectId, pareto, addParetoProject, addParetoItem, updateParetoItem, deleteParetoItem, updateParetoTitle, deleteParetoProject  } = useRoadmapStore(useShallow((state) => ({
      ...
      updateParetoTitle: state.updateParetoTitle,
      deleteParetoProject: state.deleteParetoProject
    })));
  ...
  // Proje değiştiğinde, önceki projeye ait analiz ID'sinde "takılı kalmayı" önlemek için sıfırla.
  // Aşağıdaki "Auto-select first if none selected" mantığı, bu sıfırlamadan hemen sonra
  // yeni projenin ilk analizini otomatik olarak seçecektir.
  useEffect(() => {
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          <table className="w-full text-sm text-start">
            <thead className="text-xs text-slate-500 dark:text-slate-400 uppercase bg-slate-50 dark:bg-slate-900/50">
              <tr>
                <th className="px-3 py-3 rounded-ss-lg">{t('pareto_category')}</th>
                <th className="px-3 py-3 w-24 text-end">{t('pareto_frequency')}</th>
                <th className="px-2 py-3 w-10 rounded-se-lg"></th>
              </tr>
            </thead>
            <tbody>
              {activePareto?.items.map((item) => (
                <tr key={item.id} className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-colors group">
                  ...
                  <td className="px-3 py-2">
                    {/* Sıklık her tuş vuruşunda değil, alandan çıkınca (ya da
                        Enter'la) yazılıyor. Eskiden "1250" yazmak dört ayrı
                        geçmiş adımı üretiyordu; geri tuşu rakam rakam geri
                        gidiyordu. */}
                    <DebouncedField
                      type="number"
                      min="0"
