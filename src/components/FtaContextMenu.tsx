...
export default function FtaContextMenu({ x, y, node, onClose, onAddNode, onUpdate, onDelete }: FtaContextMenuProps) {
  ...
  const handleSave = () => {
    if (editLabel.trim()) {
      let probability: number | undefined = node.data.probability;
      if (editProb.trim() === '') {
        ...
        // Geçersiz giriş (harf, aralık dışı vb.) durumunda mevcut değeri koru, sessizce silme.
      }
  ...
  return (
    ...
    <div
      ref={menuRef}
      className="context-menu fixed z-50 w-64 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden"
      style={menuStyle}
    >
      {isEditing ? (
        <div className="p-3">
