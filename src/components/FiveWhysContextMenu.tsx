...
export default function FiveWhysContextMenu({ x, y, node, onClose, onAddNode, onUpdate, onDelete }: ContextMenuProps) {
  ...
  return (
    ...
    <div
      ref={menuRef}
      className="context-menu fixed z-50 w-72 bg-white/95 dark:bg-slate-800/95 backdrop-blur-md border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
      style={menuStyle}
    >
      ...
      <div className="p-2 flex flex-col gap-1">
        {isEditing ? (
          ...
          <div className="p-2 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl space-y-2">
             <div className="text-xs font-bold text-indigo-700 dark:text-indigo-400 ps-1">
             ...
             <div className="flex justify-end gap-2">
               ...
               <button onClick={handleSaveAdd} className="px-3 py-1.5 text-xs font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors">{t('whys_add')}</button>
             </div>
          </div>
        ) : (
          <>
