...
Her görev (task) tamamlandıktan sonra, kodlar derlenecek (`npm run build`), Firebase'e dağıtılacak (`npm run build:firebase && npx firebase deploy`) ve değişiklikler KESİNLİKLE GitHub'a commitlenip push edilecektir. Kullanıcının bunu hatırlatmasına gerek kalmadan otomatik olarak "gerekli yerlere push" işlemi gerçekleştirilecektir.
